...
def create_stock_ticker_gif(stock_data, output_path="stock_ticker.gif", fps=50, speed=2):
    ...
    for name, value in stock_data.items():
        change, is_pos, eta, pop, faction = value
        
        # Format strings
        c_str = change
        c_col = up_color if is_pos else down_color
        if str(change) in ("0", "0.0", "0.0%", "0%"):
            c_col = neutral_color

        # Measure segments
        nw = temp_draw.textlength(name, font=font)
        cw = temp_draw.textlength(c_str, font=font)
        ew = temp_draw.textlength(eta, font=small_font)
        pw = temp_draw.textlength(pop, font=small_font)
        
        entry_w = int(nw + cw + ew + pw + 62 + 50) # 50px gap between planets
        prepared_entries.append({
            'name': name, 'c_str': c_str, 'eta': eta, 'pop': pop,
            'c_col': c_col, 'f_col': faction_map.get(faction, faction_map['s']),
            'w': entry_w, 'nw': nw, 'cw': cw, 'ew': ew
        })
        total_w += entry_w
    ...
    for e in prepared_entries:
        ...
        curr_x += e['cw'] + 30
        draw.text((curr_x, 24), f"{e['eta']}", fill=e['f_col'], font=small_font)
        curr_x += e['ew'] + 22
